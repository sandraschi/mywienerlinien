...
<p align="center">
  ...
  <a href="https://github.com/docsifyjs/docsify-cli/actions"><img alt="Github Actionstatus" src="https://github.com/docsifyjs/docsify-cli/workflows/docsify-cli/badge.svg"></a>
...
- `--local` option:
  ...
  - Type: boolean
  - Default: `false`
  - Description: Copy `docsify` files to the docs path, defaults to `false` using `cdn.jsdelivr.net` as the content delivery network (CDN). To explicitly sethis option to `false` use `--no-local`.
- `--theme` option:
  - Shorthand: `-t`
  - Type: string
  - Default: `vue`
...
- `--open` option:
  ...
  - Type: boolean
  - Default: `false`
  - Description: Open the docs in the default browser, defaults to `false`. To explicitly sethis option to `false` use `--no-open`.
- `--port` option:
  - Shorthand: `-p`
  - Type: number
  - Default: `3000`
...
### `generate` commandocsify's generators.
...
## Contributing
Pleasee the [Contributinguidelines](./CONTRIBUTING.md)
