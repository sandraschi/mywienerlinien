...
Returns a streamable response object similar to a [Node.js HTTP response stream](https://nodejs.org/api/http.html#http_class_http_incomingmessage). Useful formatting cached responseso they can be consumed by codexpecting a real response.
...
A Buffer containing the response body. The Buffer contents will be streamable but is also exposedirectly as `response.body`.
