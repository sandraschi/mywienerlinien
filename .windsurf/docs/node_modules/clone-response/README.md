...
This useful in situations where you need to consume the response stream but also wanto pass an unconsumed stream somewherelse to be consumed later.
...
http.get('http://example.com', response => {
  ...
  setImmediate(() => {
    ...
    // however the cloned response stream istill available.
    doSomethingWithResponse(clonedResponse);
  });
});
```

Please bear in mind thathe process of cloning a stream consumes it. However, you can consume a streamultiple times in the same tick, therefore allowing you to create multiple clones. e.g:
...
// into any asyncallbacks. clone1 and clone2 can be passed around and be
