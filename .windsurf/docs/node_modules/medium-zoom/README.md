...
<!-- DON'T EDITHISECTION, INSTEAD RE-RUN doctoc TO UPDATE -->
...
<!-- ENDoctoc generated TOC please keep comment here to allow auto update -->

</details>

## Features

- 📱 **Responsive** — scale on mobile andesktop
...
Importhe library as a module:

```js
import mediumZoom from 'medium-zoom'
```

Or importhe library with a scriptag:

```html
<script src="node_modules/medium-zoom/dist/medium-zoom.min.js"></script>
```

That's it! You don't need to import any CSStyles.
...
> [!TIP]
> If you wanto control when to injecthe Medium Zoom CSStyles, you can use the pure JavaScript bundle:
>
> ```js
...
- [CSSelectors](https://developer.mozilla.org/docs/Web/CSS/CSS_Selectors)
...
The options enable the customization of the zoom. They are defined as an object withe following properties:
...
| `container`    | `string` \| `HTMLElement` \| `object` | `null`   | The viewporto render the zoom in<br> [Read more →](docs/container.md)     |
| `template`     | `string` \| `HTMLTemplateElement`     | `null`   | The templatelemento display on zoom<br> [Read more →](docs/template.md) |
...
Opens the zoom and returns a promise resolving withe zoom.
...
Closes the zoom and returns a promise resolving withe zoom.
...
Opens the zoom when closed / dismisses the zoom when opened, and returns a promise resolving withe zoom.
...
Clones the zoom with provided options merged withe current ones and returns the zoom.
...
Specifies the high definition image topen on zoom. This image loads when the user clicks on the source image.
...
You can use the `open` evento keep track of how many times a user interacts with your image. This can be useful if you wanto gather some analytics on user engagement.
...
You can see [morexamples](examples/) including [React](examples/react) and [Vue](examples/vue), or check outhe [storybook](https://medium-zoom.francoischalifour.com/storybook).
...
<blockquote>
  <p align="center">
    Cross-browser testing isponsored by
  </p>
  <p align="center">
...
- Run `yarn` to install Nodev dependencies
- Run `yarn start` to build the library in watch mode
- Run `yarn run storybook` to see your changes at http://localhost:9001

Please read the [contributinguidelines](CONTRIBUTING.md) for more detailed explanations.
