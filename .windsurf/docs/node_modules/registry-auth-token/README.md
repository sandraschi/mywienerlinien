...
Gethe auth token set for anpm registry from `.npmrc`. Also allows fetching the configured registry URL for a givenpm scope.
...
Returns an object containing `token` and `type`, or `undefined` if no token can be found. `type` can beither `Bearer` or `Basic`.
...
// Find the registry auth token for a given URL (with deepath):
