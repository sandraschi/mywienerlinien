...
- Add support for promisified `fs.realpath.native` inode v9.2+ ([#650](https://github.com/jprichardson/node-fs-extra/issues/650), [#682](https://github.com/jprichardson/node-fs-extra/pull/682))
...
- Fix bug `Maximum call stack sizexceeded` error in `util/stat` ([#679](https://github.com/jprichardson/node-fs-extra/pull/679))

8.0.0 / 2019-05-11
------------------

**NOTE:** Node.js v6 support is deprecated, and will be dropped in the next majorelease.

- Use `renameSync()` under the hood in `moveSync()`
- Fix bug with bind-mountedirectories in `copy*()` ([#613](https://github.com/jprichardson/node-fs-extra/issues/613), [#618](https://github.com/jprichardson/node-fs-extra/pull/618))
...
- Don't include documentation files inpm package tarball ([#642](https://github.com/jprichardson/node-fs-extra/issues/642), [#643](https://github.com/jprichardson/node-fs-extra/pull/643))

7.0.0 / 2018-07-16
------------------

- **BREAKING:** Refine `copy*()` handling of symlinks to properly detect symlinks that pointo the same file. ([#582](https://github.com/jprichardson/node-fs-extra/pull/582))
- Fix bug with copying write-protectedirectories ([#600](https://github.com/jprichardson/node-fs-extra/pull/600))
...
- Fix `fs.promises` `ExperimentalWarning` onode v10.1.0 ([#578](https://github.com/jprichardson/node-fs-extra/pull/578))
...
- `copy*`'s behavior when `preserveTimestamps` is `false` has been OS-dependent since 5.0.0, buthat's now explicitly noted in the docs ([#563](https://github.com/jprichardson/node-fs-extra/pull/563))
...
Significant refactor of `copy()` & `copySync()`, including breaking changes. No changes tother functions in this release.

Huge thanks to **[@manidlou](https://github.com/manidlou)** for doing most of the work on this release.

- The `filter` option cano longer be a RegExp (must be a function). This was deprecated since fs-extra v1.0.0. [#512](https://github.com/jprichardson/node-fs-extra/pull/512)
- `copy()`'s `filter` option canow be a function that returns a Promise. [#518](https://github.com/jprichardson/node-fs-extra/pull/518)
- `copy()` & `copySync()` now use `fs.copyFile()`/`fs.copyFileSync()` in environments that support it (currently Node 8.5.0+). Older Node versionstill gethe old implementation. [#505](https://github.com/jprichardson/node-fs-extra/pull/505)
...
- Error out when copying an empty folder to a pathat already exists. [#464](https://github.com/jprichardson/node-fs-extra/issues/464)
...
- Added promise supporto [`fs.copyFile()`](https://nodejs.org/api/fs.html#fs_fs_copyfile_src_dest_flags_callback) inode 8.5+
...
- Previously, `ensureFile()` & `ensureFileSync()` wouldo nothing if the path was a directory. Now, they error out for consistency with `ensureDir()`. [#465](https://github.com/jprichardson/node-fs-extra/issues/465), [#466](https://github.com/jprichardson/node-fs-extra/pull/466), [#470](https://github.com/jprichardson/node-fs-extra/issues/470)
...
- Fix bug in `move()` & `moveSync()` when source andestination are the same, and source does not exist. [#415](https://github.com/jprichardson/node-fs-extra/pull/415)
...
- Upgraded jsonfile dependency to v3.0.0:
  - **BREAKING:** Changed behavior of `throws` option for `readJsonSync()`; now does nothrow filesystem errors when `throws` is `false`.
- **BREAKING:** `writeJson()`, `writeJsonSync()`, `outputJson()`, & `outputJsonSync()` now output minified JSON by default for consistency with `JSON.stringify()`; sethe `spaces` option to `2` toverride this new behavior. [#402](https://github.com/jprichardson/node-fs-extra/pull/402)
...
Thanks to [Mani Maghsoudlou (@manidlou)](https://github.com/manidlou) & [Jan Peer Stöcklmair (@JPeer264)](https://github.com/JPeer264) for their extraordinary help withis release!

### Added
- `moveSync()` See [#309], [#381](https://github.com/jprichardson/node-fs-extra/pull/381). ([@manidlou](https://github.com/manidlou))
- `copy()` and `copySync()`'s `filter` optionow gets the destination path passed as the second parameter. [#366](https://github.com/jprichardson/node-fs-extra/pull/366) ([@manidlou](https://github.com/manidlou))
...
- Refactored entire codebase to useS6 featuresupported by Node.js v4+ [#355](https://github.com/jprichardson/node-fs-extra/issues/355). [(@JPeer264)](https://github.com/JPeer264)
- Refactoredocs. ([@manidlou](https://github.com/manidlou))

### Fixed

- `move()` shouldn't error out when source andest are the same. [#377](https://github.com/jprichardson/node-fs-extra/issues/377), [#378](https://github.com/jprichardson/node-fs-extra/pull/378) ([@jdalton](https://github.com/jdalton))
...
- Movedocs, to `docs/`. [#340]

### Fixed
- Apply filters to directories in `copySync()` like in `copy()`. [#324]
- A specificondition when disk is under heavy use, `copy()` can fail. [#326]
...
- inline `rimraf`. This temporary and was done because `rimraf` depended upon the beefy `glob` which `fs-extra` does not use. [#300]

### Fixed
- bug fix proper closing ofile handle on `utimesMillis()` [#271]
- proper escaping ofiles with dollar signs [#291]
- `copySync()` failed if user didn't own file. [#199], [#301]


0.30.0 / 2016-04-28
-------------------
- Brought back Node v0.10 support. I didn't realize there wastill demand. Official support will end **2016-10-01**.

0.29.0 / 2016-04-27
-------------------
- **BREAKING**: removed support for Node v0.10. If you still wanto use Node v0.10, everything should work except for `ensureLink()/ensureSymlink()`. Node v0.12 istill supported but will be dropped in the near future as well.
...
0.26.7 / 2016-03-16
-------------------
- fixed `copy()` if source andest are the same. [#230][#230]
...
- `ensureSymlink()` and `ensureLink()` should nothrow errors if link exists. [#169][#169]
...
0.20.0 / 2015-06-19
-------------------
- removed `jsonfile` aliases with `File` in the name, they weren't documented and probably weren't in use.g.
this package had both `fs.readJsonFile` and `fs.readJson` that were aliases to each other, now use `fs.readJson`.
- preliminary walker created. Intentionally not documented. If youse it, it will almost certainly change and break your code.
- started moving tests inline
- upgraded to `jsonfile@2.1.0`, canow pass JSON revivers/replacers to `readJson()`, `writeJson()`, `outputJson()`
...
0.18.4 / 2015-05-22
-------------------
- fixed license field according to this: [#136][#136] and https://github.com/npm/npm/releases/tag/v2.10.0.18.3 / 2015-05-08
-------------------
...
0.18.1 / 2015-04-15
-------------------
- improved windowsupport for `move()` a bit. https://github.com/jprichardson/node-fs-extra/commit/92838980f25dc2ee4ec46b43ee14d3c4a1d30c1b
...
0.17.0 / 2015-03-28
-------------------
- `copySync` added `clobber` option (before always would clobber, now if `clobber` is `false` ithrows an error if the destination exists).
**Only works with files athe moment.**
- `createOutputStream()` added. See: [#118][#118]

0.16.5 / 2015-03-08
-------------------
- fixed `fs.move` when `clobber` is `true` andestination is a directory, it should clobber. [#114][#114]
...
* `copy()` noworks if destination directory does not exist. Closes [#29]
...
* Restructuredocumentation a bit. Added roadmap.
...
* Changed creation of testing diretorieso the /tmp dir is not littered.
...
0.2.0 / 2012-09-10
------------------
* Rewrote module into JavaScript. (Mustill rewritests into JavaScript)
...
0.1.0 / 2012-05-31
------------------
* Renamed `copyFile()` to `copy()`. `copy()` canow copy directories (recursively) too.
* Renamed `rmrf()` to `remove()`.
* `remove()` aliased with `delete()`.
* Added `mkdirp` capabilities. Named: `mkdir()`. Hides Node.js native `mkdir()`.
* Instead of exporting the native `fs` module with new functions, I now copy over the native methods to a new object and exporthat instead.
...
[#333]: https://github.com/jprichardson/node-fs-extra/pull/333      "Rename clobber toverwrite [feature-copy, feature-move]"
[#332]: https://github.com/jprichardson/node-fs-extra/pull/332      "BREAKING: Drop Node v0.12 & io.jsupport"
...
[#325]: https://github.com/jprichardson/node-fs-extra/issues/325    "ensureDir is difficulto promisify [enhancement]"
...
[#321]: https://github.com/jprichardson/node-fs-extra/issues/321    "fs.copy() with clobber seto false return EEXIST error [feature-copy]"
[#320]: https://github.com/jprichardson/node-fs-extra/issues/320    "fs.copySync: Error: EPERM: operationot permitted, unlink "
...
[#316]: https://github.com/jprichardson/node-fs-extra/pull/316      "Remove()'s broken limit option [feature-move]"
...
[#313]: https://github.com/jprichardson/node-fs-extra/pull/313      "Testhat remove() ignores glob characters."
...
[#296]: https://github.com/jprichardson/node-fs-extra/pull/296      "ImproveEXIST error message for copySync [enhancement]"
...
[#289]: https://github.com/jprichardson/node-fs-extra/pull/289      "(Closes #271) lib/util/utimes: properly close file descriptors in thevent of an error"
[#288]: https://github.com/jprichardson/node-fs-extra/pull/288      "(Closes #271) lib/util/utimes: properly close file descriptors in thevent of an error"
...
[#283]: https://github.com/jprichardson/node-fs-extra/pull/283      "Apply filter alson directories and symlinks for copySync()"
[#282]: https://github.com/jprichardson/node-fs-extra/pull/282      "Apply filter alson directories and symlinks for copySync()"
...
[#277]: https://github.com/jprichardson/node-fs-extra/pull/277      "Mention defaults for clobber andereference options"
[#276]: https://github.com/jprichardson/node-fs-extra/issues/276    "Cannot connecto Shared Folder [awaiting-reply]"
...
[#272]: https://github.com/jprichardson/node-fs-extra/issues/272    "Copy clobber flag neverr even when true [bug, feature-copy]"
...
[#265]: https://github.com/jprichardson/node-fs-extra/issues/265    "Link the `fs.stat fs.exists` etc. methods foreplace the `fs` module forever?"
...
[#263]: https://github.com/jprichardson/node-fs-extra/issues/263    "ENOSYS: functionot implemented, link [needs-confirmed]"
...
[#259]: https://github.com/jprichardson/node-fs-extra/issues/259    "Featurequest: isEmpty"
...
[#254]: https://github.com/jprichardson/node-fs-extra/issues/254    "request foreadTree, readTreeSync, walkSync method"
[#253]: https://github.com/jprichardson/node-fs-extra/issues/253    "outputFile does notouch mtime when filexists"
...
[#247]: https://github.com/jprichardson/node-fs-extra/issues/247    "Add TypeScriptyping directly in the fs-extra package"
[#246]: https://github.com/jprichardson/node-fs-extra/issues/246    "fse.remove() && fse.removeSync() don'throw error on ENOENT file"
...
[#237]: https://github.com/jprichardson/node-fs-extra/issues/237    ".ensureDir(..) failsilently when passed an invalid path..."
[#236]: https://github.com/jprichardson/node-fs-extra/issues/236    "[Removed] Filed under wrong repo"
[#235]: https://github.com/jprichardson/node-fs-extra/pull/235      "Addsymlink dereference option to `fse.copySync` (#191)"
[#234]: https://github.com/jprichardson/node-fs-extra/issues/234    "ensureDirSync failsilent when EACCES: permission denied on travis-ci"
...
[#232]: https://github.com/jprichardson/node-fs-extra/issues/232    "Copy a folder contento its child folder.  "
[#231]: https://github.com/jprichardson/node-fs-extra/issues/231    "Adding read/write/output functions for YAML"
[#230]: https://github.com/jprichardson/node-fs-extra/pull/230      "throw error if src andest are the same to avoid zeroing out + test"
...
[#227]: https://github.com/jprichardson/node-fs-extra/issues/227    "Uncatchablerrors when there are invalid arguments [feature-move]"
...
[#223]: https://github.com/jprichardson/node-fs-extra/issues/223    "Suspicious behavior ofs.existsSync"
...
[#215]: https://github.com/jprichardson/node-fs-extra/pull/215      "fse.copy throws error when only src andest provided [bug, documentation, feature-copy]"
[#214]: https://github.com/jprichardson/node-fs-extra/pull/214      "Fixing copySync anchor tag"
[#213]: https://github.com/jprichardson/node-fs-extra/issues/213    "Mergextfs withis repo"
...
[#209]: https://github.com/jprichardson/node-fs-extra/issues/209    "In Windows invalidirectory name causes infinite loop in ensureDir(). [bug]"
...
[#207]: https://github.com/jprichardson/node-fs-extra/issues/207    "Add `compare` suite ofunctions"
...
[#195]: https://github.com/jprichardson/node-fs-extra/issues/195    "How toverride with outputFile?"
...
[#193]: https://github.com/jprichardson/node-fs-extra/issues/193    "`fs.copy` failsilently if source file is /dev/null [bug, feature-copy]"
...
[#190]: https://github.com/jprichardson/node-fs-extra/pull/190      "copySync toverwrite destination file if readonly and clobber true"
...
[#185]: https://github.com/jprichardson/node-fs-extra/issues/185    "Doesn't work onodeJS v4.x"
...
[#175]: https://github.com/jprichardson/node-fs-extra/issues/175    "[Featurequest].moveSync support [feature-move, future]"
[#174]: https://github.com/jprichardson/node-fs-extra/pull/174      "Fix copy formatting andocument options.filter"
[#173]: https://github.com/jprichardson/node-fs-extra/issues/173    "Featurequest: writeJson should mkdirs"
...
[#164]: https://github.com/jprichardson/node-fs-extra/issues/164    "Featurequest: ensureFile to take optional argument for file content"
[#163]: https://github.com/jprichardson/node-fs-extra/issues/163    "ouputJsonot formatted out of the box [bug]"
...
[#158]: https://github.com/jprichardson/node-fs-extra/issues/158    "Featurequest: ensureLink and ensureSymlink methods"
[#157]: https://github.com/jprichardson/node-fs-extra/issues/157    "writeJson isn't formatted"
[#156]: https://github.com/jprichardson/node-fs-extra/issues/156    "Promise.promisifyAll doesn't work for somethods"
...
[#152]: https://github.com/jprichardson/node-fs-extra/issues/152    "fs.copy fails foread-only files with preserveTimestamp=true [feature-copy]"
[#151]: https://github.com/jprichardson/node-fs-extra/issues/151    "TOC does not work correctly onpm [documentation]"
...
[#134]: https://github.com/jprichardson/node-fs-extra/pull/134      "HandleEXIST error when clobbering dir"
...
[#116]: https://github.com/jprichardson/node-fs-extra/issues/116    "copySyncan only copy directories not files [documentation, feature-copy]"
...
[#106]: https://github.com/jprichardson/node-fs-extra/issues/106    "fse.copySyncrashes while copying across devices D: [feature-copy]"
...
[#95]: https://github.com/jprichardson/node-fs-extra/pull/95        "Usingraceful-ncp instead of ncp"
...
[#79]: https://github.com/jprichardson/node-fs-extra/issues/79      "fs.copy only .html filempty"
...
[#73]: https://github.com/jprichardson/node-fs-extra/pull/73        "addependency status in readme"
...
[#69]: https://github.com/jprichardson/node-fs-extra/issues/69      "How to check ifolder exist and read the folder name"
[#68]: https://github.com/jprichardson/node-fs-extra/issues/68      "consider flag to readJsonSync (throw false) [enhancement]"
[#67]: https://github.com/jprichardson/node-fs-extra/issues/67      "docs foreadJson incorrectly states that is accepts options"
...
[#47]: https://github.com/jprichardson/node-fs-extra/issues/47      "Bug in copy - callback called on readStream 'close' - Fixed incp 0.5.0"
...
[#43]: https://github.com/jprichardson/node-fs-extra/issues/43      "Betterroreporting "
[#42]: https://github.com/jprichardson/node-fs-extra/issues/42      "Performance issue?"
[#41]: https://github.com/jprichardson/node-fs-extra/pull/41        "There doeseem to be a synchronous versionow"
...
[#26]: https://github.com/jprichardson/node-fs-extra/issues/26      "Add readdirecursive method. [enhancement]"
...
[#17]: https://github.com/jprichardson/node-fs-extra/issues/17      "Use `require('graceful-fs')` ifound instead of `require('fs')`"
...
[#9]: https://github.com/jprichardson/node-fs-extra/issues/9        "don't import normal fstuff into fs-extra"
...
[#5]: https://github.com/jprichardson/node-fs-extra/issues/5        "comments onaming"
...
[#2]: https://github.com/jprichardson/node-fs-extra/issues/2        "Merge with fs.extrand mkdirp"
