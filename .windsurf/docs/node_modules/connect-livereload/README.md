...
connect-livereload
==================
connect middleware for adding the livereload scripto the response.
...
use
===
note: if youse this middleware, you should make sure to switch off the Browser LiveReload Extension if you have it installed.

this middleware can be used with a LiveReload module.g. [grunt-contrib-connect](https://github.com/gruntjs/grunt-contrib-connect) or [grunt-contrib-watch](https://github.com/gruntjs/grunt-contrib-watch).
...
pleasee the [examples](https://github.com/intesso/connect-livereload/tree/master/examples) for the app and Grunt configuration.
...
These are the available options withe following defaults:

```javascript
  // `ignore` and `include`: array with strings and regexpressions elements.
  // strings: included/ignored when the url contains thistring
  ...
  // the function `fn` has gothe arguments `fn(w, s)` where `w` is the matchestring and `s` is the snippet.
  rules: [{
  ...
  // Sethis option to `true` to set `req.headers['accept-encoding']` to 'identity' (disabling compression)
  disableCompression: false,

  // Locations where livereload plugins are provided (not by connect-livereload).
  // These pluginshould handle being loaded before _or_ after the livereload
...
```
You can pass in options to this call if you do not wanthe defaults.

`dev` isimply the name of the server being used withe task `grunt connect:dev`. The other items in the `middleware` array are all functions that either are of the form `function (req, res, next)` like `checkForDownload` oreturn that like `mountFolder(connect, 'something')`.
