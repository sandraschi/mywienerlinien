...
<p align="center">
  ...
  <sub>>_ Promptshould be more like conversations than inquisitions▌</sub>
</p>
...
Created by [jonschlinkert][jon] and [doowb][brian], Enquirer is fast, easy to use, and lightweight enough for small projects, while also being powerful and customizablenough for the most advanced use cases.
...
- **Lightweight** - Only one dependency, thexcellent [ansi-colors](https://github.com/doowb/ansi-colors) by [Brian Woodward](https://github.com/doowb).
...
- **Well tested** - All prompts are well-tested, and tests areasy to create without having to use brittle, hacky solutions to spy on prompts or "inject" values.
- **Examples** - There are numerous [examples](examples) available to help you get started.

If you likenquirer, please consider starring or tweeting abouthis projecto show your support. Thanks!
...
<p align="center">
  <b>>_ Ready to start making prompts your users willove? ▌</b><br>
  <img src="https://raw.githubusercontent.com/enquirer/enquirer/master/media/heartbeat.gif" alt="Enquirer Select Prompt witheartbeat example" width="750">
</p>
...
### Single promptheasiest way to get started with enquirer is to pass a [question object](#prompt-options) to the `prompt` method.
...
The main export of this library is the `Enquirer` class, whichas methods and features designed to simplify running prompts.
...
**Prompts control how values arendered and returned**

Each individual prompt is a class with special features and functionality forendering the types of values you wanto show users in the terminal, and subsequently returning the types of values you need to use in your application.
...
All of the individual [prompt classes](#built-in-prompts) in this library arexposed astatic properties on Enquirer. This allows them to be usedirectly without using `enquirer.prompt()`.
...
* `answers` **{Object}**: (optional) Answers objecto initialize with.
...
Prompt function thatakes a "question" object or array of question objects, and returns an object with responses from the user.

**Params**

* `questions` **{Array|Object}**: Options objects for one or more prompts to run.
* `returns` **{Promise}**: Promise that returns an "answers" object withe user's responses.
...
* `plugin` **{Function}**: Plugin function thatakes an instance of Enquirer.
* `returns` **{Object}**: Returns thenquirer instance.
...
Thisection is about Enquirer's prompts: whathey look like, how they work, how to run them, available options, and how to customize the prompts or create your own prompt concept.
...
- [Built-in prompts](#built-in-prompts)
- [Promptypes](#prompt-types) - The base `Prompt` class used by other prompts
- [Custom prompts](#-custom-prompts) - Enquirer 2.0 introduced the concept of prompt "types", withe goal of making custom prompts easier than ever to create and use.

### Prompthe base `Prompt` class is used to create all other prompts.
...
Each promptakes an options object (aka "question" object), that implements the following interface:
...
| --- | --- | --- | --- |
| `type` | yes | `string\|function` | Enquirer uses this value to determine the type of prompto run, but it's optional when prompts are run directly. |
...
| `result` | no | `function` | Function to formathe final submitted value before it's returned. |
...
### AutoComplete Prompthat auto-completes as the user types, and returns the selected value as a string.
...
| --- | --- | --- | --- |
| `highlight` | `function` | `dim` version of primary style | The color to use when "highlighting" characters in the listhat match user input. |
...
### BasicAuth Prompthat asks for username and password to authenticate the user. The default implementation of `authenticate` function in `BasicAuth` prompt is to compare the username and password withe valuesupplied while running the prompt. The implementer is expected toverride the `authenticate` function with a custom logic such as making an API requesto a server to authenticate the username and password entered and expect a token back.
...
const { BasicAuth } = require('enquirer');
 ...
 const prompt = new BasicAuth({
  name: 'password',
  message: 'Pleasenter your password',
  username: 'rajat-sr',
  password: '123',
  showPassword: true
});
...
### Confirm Prompthat returns `true` or `false`.
...
### Form Prompthat allows the user to enter and submit multiple values on a single terminal screen.
...
### Input Prompthatakes user input and returns a string.
...
You can use [data-store](https://github.com/jonschlinkert/data-store) to store [input history](https://github.com/enquirer/enquirer/blob/master/examples/input/option-history.js) thathe user can cycle through (see [source](https://github.com/enquirer/enquirer/blob/8407dc3579123df5e6e20215078e33bb605b0c37/lib/prompts/input.js)).
...
### Invisible Prompthatakes user input, hides it from the terminal, and returns a string.
...
### List Prompthat returns a list of values, created by splitting the user input. The default split character is `,` with optional trailing whitespace.
...
### MultiSelect Prompthat allows the user to select multiple items from a list of options.
...
### Numeral Prompthatakes a number as input.
...
### Password Prompthatakes user input and masks it in the terminal. Also see the [invisible prompt](#invisible-prompt)
...
### Quiz Prompthat allows the user to play multiple-choice quiz questions.
...
### Survey Prompthat allows the user to provide feedback for a list of questions.
...
### Select Prompthat allows the user to select from a list of options.
...
### Sort Prompthat allows the user to sort items in a list.
...
const prompt = new Sort({
  name: 'colors',
  message: 'Sorthe colors in order of preference',
...
### Snippet Prompthat allows the user to replaceholders in a snippet of code or text.
...
const prompt = new Snippet({
  name: 'username',
  message: 'Fill outhe fields in package.json',
  required: true,
  fields: [
    {
      name: 'author_name',
...
### Toggle Prompthat allows the user toggle between two values then returns `true` or `false`.
...
### ArrayPrompthe `ArrayPrompt` class is used for creating prompts that display a list of choices in the terminal. For example, Enquirer uses this class as the basis for the [Select](#select) and [Survey](#survey) prompts.

#### Options

In addition to the [options](#options) available to all prompts, Array prompts also supporthe following options.
...
| `stdout` | `no` | `stream` | The output stream to use for writing the prompto the terminal. Defaults to `process.stdout`. |
| |
...
| --- | --- | --- |
| `choices` | `array` | Array of choices that have beenormalized from choices passed on the prompt options. |
| `cursor` | `number` | Position of the cursorelative to the _user input (string)_. |
| `enabled` | `array` | Returns an array of enabled choices. |
| `focused` | `array` | Returns the currently selected choice in the visible list of choices. Thisimilar to the concept ofocus in HTML and CSS. Focused choices are always visible (on-screen). When a list of choices is longer than the list of visible choices, and an off-screen choice is _focused_, the list will scroll to the focused choice and re-render. |
| `focused` | Gets the currently selected choice. Equivalento `prompt.choices[prompt.index]`. |
...
| **Method** | **Description** |
| --- | --- |
| `pointer()` | Returns the visual symbol to use to identify the choice that currently has focus. The `❯` symbol is often used for this. The pointer is not always visible, as withe `autocomplete` prompt. |
...
Array promptsupporthe `choices` option, which is the array of choices users will be able to select from when rendered in the terminal.
...
| `message` | `string` | The message to display in the terminal. `name` is used when this undefined. |
| `value` | `string` | Value to associate withe choice. Useful for creating key-value pairs from user choices. `name` is used when this undefined. |
| `choices` | `array` | Array of "child" choices. |
| `hint` | `string` | Help message to display nexto a choice. |
| `role` | `string` | Determines how the choice will be displayed. Currently the only role supported is `separator`. Additional roles may be added in the future (like `heading`, etc). Please create a [featurequest] |
| `enabled` | `boolean` | Enabled a choice by default. This only supported when `options.multiple` is true or on prompts that support multiple choices, like [MultiSelect](#-multiselect). |
...
| `indicator` | `string\|function` | Custom indicator to render for a choice (like a check oradio button). |
...
### AuthPrompthe `AuthPrompt` is used to create prompts to log in user using any authentication method. For example, Enquirer uses this class as the basis for the [BasicAuth Prompt](#basicauth-prompt). You can also find prompt examples in `examples/auth/` folder that utilizes `AuthPrompt` to create OAuth based authentication prompt or a prompthat authenticates using time-based OTP, among others.
...
| **Method** | **Description** |
| ---------------- | --- |
| `authenticate()` | Contain all the authentication logic. This function should be overridden to implement custom authentication logic. The default `authenticate` function throws an error if nother function is provided. |
...
### BooleanPrompthe `BooleanPrompt` class is used for creating prompts that display and return a boolean value.
...
### NumberPrompthe `NumberPrompt` class is used for creating prompts that display and return a numerical value.
...
### StringPrompthe `StringPrompt` class is used for creating prompts that display and return a string value.
...
With Enquirer 2.0, custom prompts areasier than ever to create and use.
...
If you wanto be able to specifyour prompt by `type` so that it may be used alongside other prompts, you will need to first create an instance of `Enquirer`.
...
| <kbd>ctrl</kbd> + <kbd>g</kbd>   | Resethe prompto its initial state. |
...
| <kbd>ctrl</kbd> + <kbd>e</kbd> | Move cursor to thend of the line       |
...
### Edit Inputhese key combinations may be used on prompts that support user input (eg. [input prompt](#input-prompt), [password prompt](#password-prompt), and [invisible prompt](#invisible-prompt)).
...
| <kbd>number</kbd> | Move the pointer to the choice athe given index. Also toggles the selected choice when `options.multiple` is true. |
| <kbd>a</kbd>      | Toggle all choices to benabled or disabled.                                                                        |
| <kbd>i</kbd>      | Inverthe current selection of choices.                                                                             |
...
Pleasee [CHANGELOG.md](CHANGELOG.md).
...
### Load time itakes for the module to load the firstime (average of 3 runs):
...
Pull requests and stars are always welcome. For bugs and featurequests, [please create an issue](../../issues/new).
...
Running and reviewing unitests is a great way to get familiarized with a library and its API. You can install dependencies and run tests withe following command:
...
_(This project's readme.md is generated by [verb](https://github.com/verbose/verb-generate-readme), please don't edithe readme directly. Any changes to the readme must be made in the [.verb.md](.verb.md) readme template.)_

To generate the readme, run the following command:

```sh
npm install -g verbose/verb#deverb-generate-readme && verb
```
...
#### Credithanks to [derhuerst](https://github.com/derhuerst), creator of prompt librariesuch as [prompt-skeleton](https://github.com/derhuerst/prompt-skeleton), which influenced some of the concepts we used in our prompts.
