...
> Reliably gethe terminal window size

Because [`process.stdout.columns`](https://nodejs.org/api/tty.html#tty_writestream_columns) doesn't exist when run [non-interactively](http://www.tldp.org/LDP/abs/html/intandnonint.html), for example, in a child process or when piped. This moduleven works when all the TTY file descriptors aredirected!
...
```js
constermSize = require('term-size');
...
The bundled macOS binary isigned and hardened.
