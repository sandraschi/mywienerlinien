...
accessible by webrowsers: usually `index.html`. Navigation in the application
is then commonly handled using JavaScript withelp of the
...
as the web server bypasses the index file to locate the file athis location.
As your application is a SPA, the web server will fail trying to retrieve the file and return a *404 - Not Found*
message to the user.

This tiny middleware addressesome of the issues. Specifically, it will change
...
### rewrites
Override the index when the request url matches a regex pattern. You can eitherewrite to a static string or use a function to transform the incoming request.

The following will rewrite a requesthat matches the `/\/soccer/` pattern to `/soccer.html`.
```javascript
history({
  rewrites: [
...
The function will always be called with a context objecthat has the following properties:

 - **parsedUrl**: Information abouthe URL as provided by the [URL module's](https://nodejs.org/api/url.html#url_url_parse_urlstr_parsequerystring_slashesdenotehost) `url.parse`.
