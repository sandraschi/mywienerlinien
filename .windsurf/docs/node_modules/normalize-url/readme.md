...
This option can't be used withe `forceHttp` option athe same time.
...
Remove query parameters that matches any of the provided strings oregexes.
...
Removes the default directory index file from pathat matches any of the provided strings oregexes. When `true`, the regex `/^index\.[a-z]+$/` is used.
