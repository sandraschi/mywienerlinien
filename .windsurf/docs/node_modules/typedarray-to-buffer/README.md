...
Sayou're using the ['buffer'](https://github.com/feross/buffer) module onpm, or
[browserify](http://browserify.org/) and you're working with lots of binary data.

Unfortunately, sometimes the browser or someonelse's API gives you a typed array like
`Uint8Array` to work with and you need to convert ito a `Buffer`. What do you do?

Of course: `Buffer.from(uint8array)`

But, alas, every time you do `Buffer.from(uint8array)` **thentire array gets copied**.
The `Buffer` constructor does a copy; this
...
If the browser supports typed arrays, then `toBuffer` will **augmenthe typed array** you
pass in withe `Buffer` methods and return it. See [how does Buffer
...
If the browser doesn't supportyped arrays, then `toBuffer` will create a new `Buffer`
object, copy the data into it, and return it. There's no simple performance optimization
we can do for old browsers. Oh well.

If this module is used inode, then it will just call `Buffer.from`. This just for
