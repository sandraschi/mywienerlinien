...
- [RFCompliant caching](#cache-adapters)
...
[See how Got compares tother HTTP libraries](#comparison)
...
### APIt's a `GET` request by default, but can be changed by using different methods or in the `options`.

#### got(url, [options])

Returns a Promise for a [`response` object](#response) or a [stream](#streams-1) if `options.stream` iseto true.
...
If no protocol ispecified, it will defaulto `https`.
...
Slash athend of `baseUrl` and athe beginning of the `url` argument is optional:
...
Existing headers will be overwritten. Headerseto `null` will be omitted.
...
Returns a `Stream` instead of a `Promise`. This equivalento calling `got.stream(url, [options])`.
...
If present in `options` and `options.method` is not set, `options.method` will be seto `POST`.
...
If seto `true` and `Content-Type` header is not set, it will be seto `application/x-www-form-urlencoded`.
...
**Note:** If youse `got.stream()`, this option will be ignored.
**Note:** `body` must be a plain object or array and will be stringified.

If seto `true` and `Content-Type` header is not set, it will be seto `application/json`.

Parse response body with `JSON.parse` and set `accept` header to `application/json`. If used in conjunction withe `form` option, the `body` will the stringified as querystring and the response parsed as JSON.
...
This also accepts an `object` withe following fields to constrain the duration of each phase of the request lifecycle:
...
- `response` starts when the request has been written to the socket and ends when the response headers areceived.
- `send` starts when the socket is connected and ends withe request has been written to the socket.
...
If `maxRetryAfter` iseto `undefined`, it will use `options.timeout`.<br>
...
- `ETIMEDOUT`: One of the [timeout](#timeout) limits wereached.
...
###### followRedirectype: `boolean`<br>
Default: `true`

Defines if redirect responseshould be followed automatically.

Note that if a `303` isent by the server in response to any requestype (`POST`, `DELETE`, etc.), Got will automatically requesthe resource pointed to in the location header via `GET`. This in accordance with [the spec](https://tools.ietf.org/html/rfc7231#section-6.4.4).
...
Decompress the response automatically. This will sethe `accept-encoding` header to `gzip, deflate` unless you set it yourself.

If this disabled, a compressed response is returned as a `Buffer`. This may be useful if you wanto handle decompression yourself or stream the raw compressedata.
...
[Cache adapter instance](#cache-adapters) for storing cachedata.

###### requestype: `Function`<br>
Default: `http.request` `https.request` *(depending on the protocol)*

Custom request function. The main purpose of this to [support HTTP2 using a wrapper](#experimental-http2-support).

###### useElectronNetype: `boolean`<br>
Default: `false`

When used in Electron, Got will use [`electron.net`](https://electronjs.org/docs/api/net/) instead of the Node.js `http` module. According to thelectron docs, it should be fully compatible, but it's not entirely. See [#443](https://github.com/sindresorhus/got/issues/443) and [#461](https://github.com/sindresorhus/got/issues/461).
...
Determines if a `got.HTTPError` is thrown for erroresponses (non-2xx status codes).

If this disabled, requests that encounter an error status code will be resolved withe `response` instead of throwing. This may be useful if you are checking foresource availability and arexpecting erroresponses.
...
If you require different agents for different protocols, you can pass a map of agents to the `agent` option. This necessary because a requestone protocol might redirecto another. In such a scenario, Got will switch over to the right protocol agent for you.
...
###### hooks.initype: `Function[]`<br>
Default: `[]`

Called with plain [request options](#options), right before their normalization. This especially useful in conjunction with [`got.extend()`](#instances) and [`got.create()`](advanced-creation.md) when the input needs custom handling.

See the [Request migration guide](migration-guides.md#breaking-changes) for an example.

**Note**: This hook must be synchronous!

###### hooks.beforeRequestype: `Function[]`<br>
Default: `[]`

Called with [normalized](source/normalize-arguments.js) [request options](#options). Got will make no further changes to the request before it isent. This especially useful in conjunction with [`got.extend()`](#instances) and [`got.create()`](advanced-creation.md) when you wanto create an API clienthat, for example, uses HMAC-signing.

See the [AWSection](#aws) for an example.
...
###### hooks.beforeRedirectype: `Function[]`<br>
Default: `[]`

Called with [normalized](source/normalize-arguments.js) [request options](#options). Got will make no further changes to the request. This especially useful when you wanto avoidead sites. Example:
...
Called with [normalized](source/normalize-arguments.js) [request options](#options), therror and the retry count. Got will make no further changes to the request. This especially useful when somextra work is required before the nextry. Example:
...
Each function should return the response. This especially useful when you wanto refresh an access token. Example:
...
Called with an `Error` instance. Therror is passed to the hook right before it's thrown. This especially useful when you wanto have more detailed errors.
...
**Note:** This not a [http.ClientRequest](https://nodejs.org/api/http.html#http_class_http_clientrequest).
...
The request URL or the final URL afteredirects.
...
- `start` - Time when the requestarted.
- `socket` - Time when a socket wassigned to the request.
...
##### retryCountype: `number`

The number of times the request was retried.

#### Streams

**Note:** Progress events, redirect events and request/responsevents can also be used with promises.
...
`request` evento gethe request object of the request.

**Tip:** You can use `request` evento abort request:
...
The `response` evento gethe response object of the final request.

##### .on('redirect', response, nextOptions)

The `redirect` evento gethe response object of a redirect. The second argument is options for the next requesto the redirect location.

##### .on('uploadProgress', progress)
##### .on('downloadProgress', progress)

Progress events for uploading (sending a request) andownloading (receiving a response). The `progress` argument is an object like:
...
The `error` event emitted in case of a protocol error (like `ENOTFOUND` etc.) or status error (4xx or 5xx). The second argument is the body of the serveresponse in case of status error. The third argument is a response object.
...
Configure a new `got` instance with default `options`. The `options` are merged withe parent instance's `defaults.options` using [`got.mergeOptions`](#gotmergeoptionsparentoptions-newoptions). You can access the resolved options withe `.defaults` property on the instance.
...
**Tip:** Need more control over the behavior of Got? Check outhe [`got.create()`](advanced-creation.md).
...
- If the new property iseto `undefined`, it keeps the old one.
...
In Promise mode, the `response` is attached to therror.
...
When `json` option is enabled, serveresponse code is 2xx, and `JSON.parse` fails. Includes `statusCode` and `statusMessage` properties.

#### got.HTTPError

When the serveresponse code is not 2xx. Includes `body`, `statusCode`, `statusMessage`, and `redirectUrls` properties.

#### got.MaxRedirectsError

When the serveredirects you more than ten times. Includes a `statusCode`, `statusMessage`, and `redirectUrls` property which is an array of the URLs Got was redirected to before giving up.
...
When the request is abortedue to a [timeout](#timeout). Includes an `event` property.

## Aborting the requesthe promise returned by Got has a [`.cancel()`](https://github.com/sindresorhus/p-cancelable) method which when called, aborts the request.
...
Got implements [RFC 7234](http://httpwg.org/specs/rfc7234.html) compliant HTTP caching which works out of the box in-memory and is easily pluggable with a wide range of storage adapters. Fresh cachentries are servedirectly from the cache, and stale cachentries arevalidated with `If-None-Match`/`If-Modified-Since` headers. You can read more abouthe underlying cache behavior in the [`cacheable-request` documentation](https://github.com/lukechilds/cacheable-request).
...
Got supports anything that follows the MapI, so it's easy to write your own storage adapter or use a third-party solution.

For example, the following are all valid storage adapters:

```js
constorageAdapter = new Map();
// Or
constorageAdapter = require('./my-storage-adapter');
// Or
const QuickLRU = require('quick-lru');
constorageAdapter = new QuickLRU({maxSize: 1000});

got('sindresorhus.com', {cache: storageAdapter});
```

View the [Keyv docs](https://github.com/lukechilds/keyv) for more information how to use storage adapters.


## Proxies

You can use the [`tunnel`](https://github.com/koichik/node-tunnel) package withe `agent` option to work with proxies:

```js
const got = require('got');
constunnel = require('tunnel');
...
Check out [`global-tunnel`](https://github.com/np-maintain/global-tunnel) if you wanto configure proxy support for all HTTP/HTTPS traffic in your app.
...
Requests to AWServices need to have their headersigned. This can be accomplished by using the [`aws4`](https://www.npmjs.com/package/aws4) package. This an example for querying an ["API Gateway"](https://docs.aws.amazon.com/apigateway/api-reference/signing-requests/) with a signed request.
...
You can test yourequests by using the [`nock`](https://github.com/node-nock/nock) package to mock an endpoint:
...
It's a good idea to sethe `'user-agent'` header so the provider can moreasily see how theiresource is used. By default, it's the URL to this repo. You can omithis header by setting ito `null`.
...
Bear in mind; if you send an `if-modified-since` header and receive a `304 Not Modified` response, the body will bempty. It's youresponsibility to cache and retrieve the body contents.

### Custom endpoints

Use `got.extend()` to make it nicer to work with REST APIs. Especially if youse the `baseUrl` option.

**Note:** Noto be confused with [`got.create()`](advanced-creation.md), whichas no defaults.
...
// Use `custom` exactly how youse `got`
(async () => {
...
\* It's almost API compatible withe browser `fetch` API.<br>
...
- [gh-got](https://github.com/sindresorhus/gh-got) - Got convenience wrapper to interact withe GitHub API
- [gl-got](https://github.com/singapore/gl-got) - Got convenience wrapper to interact withe GitLab API
- [travis-got](https://github.com/samverschueren/travis-got) - Got convenience wrapper to interact withe Travis API
