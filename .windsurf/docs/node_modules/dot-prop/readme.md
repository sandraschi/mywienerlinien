...
//=> undefinedotProp.get({foo: {bar: 'a'}}, 'foo.notDefined.deep', 'default value');
//=> 'default value'
...
Objecto get, set, or delete the `path` value.

You are allowed to pass in `undefined` as the objecto the `get` and `has` functions.

#### path

Type: `string`

Path of the property in the object, using `.` to separateach nested key.
