...
def load_stops(gtfs_dir: str) -> dict[str, Stop]:
    ...
    try:
        with open(stops_file, encoding="utf-8-sig") as f:  # Handle BOM if present
...
def load_routes(gtfs_dir: str) -> dict[str, Route]:
    ...
    try:
        with open(routes_file, encoding="utf-8-sig") as f:  # Handle BOM if present
            reader = csv.DictReader(f)
            for row in reader:
                try:
                    route = Route(
...
def load_trips(gtfs_dir: str, routes: dict[str, Route]) -> dict[str, dict]:
    ...
    try:
        with open(trips_file, encoding="utf-8-sig") as f:  # Handle BOM if present
            reader = csv.DictReader(f)
            for row in reader:
                try:
                    route_id = row["route_id"].strip()
...
def process_stop_times(
    ...
    try:
        ...
        with open(stop_times_file, encoding="utf-8-sig") as f:  # Handle BOM if present
            reader = csv.DictReader(f)
            for row in reader:
                try:
                    processed_rows += 1
...
def generate_route_markdown(routes: dict[str, Route], route_type: str, output_dir: str) -> None:
    ...
    for route in filtered_routes:
        logger.info(
            f"Processing route: {route.route_short_name} ({route.route_id}), Directions: {len(route.directions)}"
        )
    ...
    filtered_routes = [r for r in routes.values() if r.route_type == route_type and r.directions]
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"*Generated from official Wiener Linien GTFS data*  *Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*\n\n"
        )
...
def main():
    ...
    route_types = {"0": "tram", "1": "u_bahn", "3": "bus"}
