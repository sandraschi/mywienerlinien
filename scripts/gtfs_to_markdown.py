...
class GTFSMarkdownGenerator:
    ...
    def generate_route_markdown(
        self, route_type: str, routes: list[pygtfs.gtfs_entities.Route]
    ) -> str:
        """Generate markdown content for routes of a specific type."""
        if not routes:
            return ""

        route_type_info = self.route_types.get(
            route_type,
            {
    ...
    def generate_station_markdown(self) -> str:
        ...
        for stop in self.schedule.stops:
            ...
            for route in (
                self.session.query(pygtfs.gtfs_entities.Route)
        ...
        for station_id, data in sorted_stations:
            ...
            location = (
                f"{data['location'][0]:.6f}, {data['location'][1]:.6f}"
                if data["location"]
    ...
    def generate_trip_markdown(
        ...
        for route in sorted(routes, key=lambda r: r.route_short_name):
            ...
            for headsign, trips in trips_by_headsign.items():
                ...
                for trip in sorted(
                    trips[:5],
                    key=lambda t: t.departure_time if hasattr(t, "departure_time") else "",
                ):
    ...
    def generate_all(self):
        ...
        try:
            ...
            for route_type, routes in routes_by_type.items():
                ...
                trip_file = self.output_dir / route_type_info.get(
                    "trip_file", f"type_{route_type}_trips.md"
                )
