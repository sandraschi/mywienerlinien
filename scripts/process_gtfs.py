...
class GTFSProcessor:
    ...
    def download_file(self, url: str, output_path: Path, timeout: int = 60) -> bool:
        ...
        try:
            ...
            with DownloadProgressBar(
                ...
                try:
                    urllib.request.urlretrieve(
                        url, filename=output_path, reporthook=t.update_to, data=None
                    )
            ...
            if output_path.exists() and output_path.stat().st_size > 0:
                logger.info(
                    f"Successfully downloaded {output_path.name} ({output_path.stat().st_size / (1024 * 1024):.2f} MB)"
                )
                return True
            else:
    ...
    def load_gtfs_data(self, gtfs_dir: Path) -> dict[str, Any]:
        ...
        def load_file(file_path, chunked=False, chunk_size=None, file_handle=None):
            ...
            try:
                if file_handle is None:
                    ...
                    file_handle = open(file_path, encoding="utf-8-sig")
                ...
                if not chunked:
                    ...
                    try:
                        ...
                        if (
                            file_handle not in file_handles
                        ):  # Don't close if tracked for chunked reading
                            file_handle.close()
                else:
        ...
        if not extracted_dir.exists() or not extracted_dir.is_dir():
            ...
            logger.info(
                f"Available directories: {[d.name for d in gtfs_dir.iterdir() if d.is_dir()]}"
            )
            return {}
        ...
        if missing_required:
            logger.error(
                f"Missing {len(missing_required)} required GTFS files: {', '.join(missing_required)}"
            )
            available_files = [f.name for f in extracted_dir.glob("*") if f.is_file()]
            logger.info(
                f"Available files in {extracted_dir}: {', '.join(available_files) if available_files else 'None'}"
            )
            return {}
        ...
        for file_name in files_to_load:
            ...
            file_data = load_file(
                file_path, chunked=config.get("chunked", False), chunk_size=config.get("chunk_size")
            )
    ...
    def _build_route_directions(self, routes, trips, stop_times, stops, station_variants=None):
        ...
        for trip_id, trip in trips.items():
            ...
            if direction_id not in routes[route_id]["directions"]:
                routes[route_id]["directions"][direction_id] = {"stops": [], "trip_ids": set()}
        ...
        return {"routes": routes, "stops": stops, "station_variants": station_variants}
    ...
    def generate_markdown(self, data: dict, output_dir: Path):
        ...
        for i, (route_id, route) in enumerate(list(routes.items())[:3]):
            logger.info(
                f"[DEBUG] Route {i + 1}: {route_id} - {route.get('route_short_name')} ({route.get('route_long_name')})"
            )
        ...
        for route_id, route in routes.items():
            ...
            if not route_type:
                logger.warning(
                    f"Unknown route type: {route_type_id} for route {route.get('route_short_name', route_id)}"
                )
                continue
        ...
        for route_type_id, type_info in self.route_types.items():
            ...
            sorted_routes = sorted(
                type_routes.values(), key=lambda x: x.get("route_short_name", "")
            )
            ...
            try:
                with open(output_file, "w", encoding="utf-8") as f:
                    ...
                    for route in sorted_routes:
                        ...
                        for direction_id, direction in sorted(route.get("directions", {}).items()):
                            ...
                            if not dir_label:
                                dir_label = (
                                    "Inbound"
                                    if str(direction_id) in ("0", "inbound")
                                    else "Outbound"
                                )
                            ...
                            f.write(
                                f"### Direction: {dir_label} (Trip ID: {representative_trip_id})\n"
                            )
                    ...
                    for route_id in stop_routes.keys():
                        ...
                        if type_info:
                            route_types.append(
                                type_info.get("title", type_info.get("name", "Unknown"))
                            )
                    if not route_types:
                        route_types = ["Unknown"]
                    station_type = route_types[0]
                    f.write(
                        f"{i}. **{stop_name}{variant_note}** - RBL: {rbl_str}, Type: {station_type}, Zone: {zone}  \n"
                    )
    ...
    def _generate_station_files(
        self, routes: dict, stops: dict, station_variants: dict, output_dir: Path
    ):
                ...
                with open(station_file, "w", encoding="utf-8") as f:
                    ...
                    for stop_name in sorted(stops_by_name.keys()):
                        ...
                        for stop in stop_group:
                            variant_note = " (Variant)" if stop.get("is_variant") else ""
                            f.write(
                                f"- **Stop ID**: {stop.get('stop_id', 'N/A')}{variant_note}  \n"
                            )
                            ...
                            lat_str = (
                                ...
                                else stop.get("stop_lat", "N/A")
                            )
                            lon_str = (
                                f"{lon:.6f}"
                                if isinstance(lon, (int, float))
                                else stop.get("stop_lon", "N/A")
                            )
                            f.write(f"  - Coordinates: {lat_str}, {lon_str}  \n")
                            rbl_numbers = stop.get("rbl_numbers") or []
...
def main():
    try:
        ...
        try:
            ...
            logger.info(
                f"Processed data: {len(processed_data.get('routes', {}))} routes, "
            ...
            if output_files:
                logger.info(
                    f"Generated {len(output_files)} markdown files: {[f.name for f in output_files]}"
                )
            else:
