...
try:
    from .city_config import CityConfig, get_city_config, get_gtfs_filename, list_cities
except ImportError:
    from city_config import (  # type: ignore
        CityConfig,
        get_city_config,
        get_gtfs_filename,
        list_cities,
...
def get_gtfs_path(city_name: str | None = None, gtfs_url: str | None = None) -> Path:
    ...
    for route in gtfs_data.get("routes.txt", []):
        try:
            ...
            if route_key in seen_route_keys:
                print(
                    f"  • Skipping duplicate route: {route_short_name} - {route_long_name} (Type: {route_type})"
                )
                continue
...
def process_stops(gtfs_data: dict, max_stops: int = None) -> dict:
    ...
    for i, stop in enumerate(stops_data, 1):
        try:
            ...
            if i % 10000 == 0 or (current_time - last_log_time) >= 5 or i == len(stops_data):
                ...
                print(
                    f"  • Processed {i:,}/{len(stops_data):,} stops "
    ...
    print(
        f"  • Successfully processed {len(stops):,} stops in "
...
def process_stop_times(
    gtfs_data: dict, routes: dict, trips: dict, stops: dict, max_entries: int = None
):
    ...
    for i, stop_time in enumerate(stop_times, 1):
        ...
        if i % 100000 == 0 or (current_time - last_log_time) >= 5 or i == total_entries:
            ...
            print(
                f"    - Processed {i:,}/{total_entries:,} entries "
    ...
    print(
        f"  • Assigned {total_stops_assigned} stops to {routes_with_stops} out of {len(routes)} routes"
    )
    ...
    for i, (route_id, route) in enumerate(routes.items(), 1):
        ...
        if i % 100 == 0 or i == route_count:
            ...
            print(
                f"    - Sorted {i:,}/{route_count:,} routes: {route_id} has {len(route_stops)} stops ({directions} directions)"
            )
    ...
    elapsed = time.time() - start_time
    print(
        f"  • Processed {processed:,} stop time entries "
...
def generate_markdown_files(routes: dict, stops: dict, output_dir: Path):
    ...
    for route_id, route in routes.items():
        ...
        if (
            route_type_info
            and route_type_info["name"] == "bus"
            and route["route_short_name"].startswith(("N", "n"))
        ):
    ...
    for route_type_id, type_info in route_types.items():
        ...
        print(
            f"  • Generating {type_info['filename']} with {len(type_routes)} {type_info['title']} routes..."
        )
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            f.write(
                "*Generated from official Wiener Linien GTFS data*  "
            ...
            if len(type_routes) > 5:  # Only add TOC for larger files
                ...
                for route in type_routes:
                    ...
                    f.write(
                        f"- [{route_name}](#{route['route_short_name'].lower().replace(' ', '-')}---{route['route_long_name'].lower().replace(' ', '-').replace('/', '')})\n"
                    )
                f.write("\n")
                ...
                if len(stops_by_direction) > 1:
                    ...
                    for direction_id in sorted(stops_by_direction.keys()):
                        ...
                        if dir_stops:
                            ...
                            directions.append(
                                f"Direction {direction_id}: {first_stop} → {last_stop}"
                            )
                ...
                if len(stops_by_direction) > 1:
                    for direction_id in sorted(stops_by_direction.keys()):
                        ...
                        for i, stop in enumerate(dir_stops, 1):
                            ...
                            f.write(
                                f"   - Coordinates: {stop.get('lat', 0):.6f}, {stop.get('lon', 0):.6f}  \n"
                            )
    ...
    for stop_id, stop in stops.items():
        ...
        if location_type == 1 and (
            ...
            or "U-Bahn" in name
            or " U " in f" {name} "
            or any(word.startswith("U") and len(word) <= 3 for word in name.split())
        ):
    ...
    if all_stations:
        ...
        with open(output_file, "w", encoding="utf-8") as f:
            ...
            sorted_stations = sorted(all_stations.items(), key=lambda x: (x[0].lower(), x[0]))
            ...
            for letter, group in groupby(
                sorted_stations, key=lambda x: x[0][0].upper() if x[0] else "#"
            ):
                stations = list(group)
                if stations:
...
def parse_arguments():
    ...
    parser.add_argument(
        "--force-extract",
        action="store_true",
        help="Force extraction of GTFS data even if it exists",
    )
    parser.add_argument(
        "--full", action="store_true", help="Process the full dataset (may take a long time)"
    )
    parser.add_argument(
        "--max-entries",
        type=int,
...
def main():
    ...
    try:
        ...
        if not is_gtfs_fresh(gtfs_zip) or args.force_download:
            if gtfs_zip.exists():
                if args.force_download:
                    ...
                    print(
                        f"  • GTFS data is older than {GTFS_MAX_AGE_DAYS} days, downloading fresh copy..."
                    )
            else:
        ...
        gtfs_data = process_with_timeout(
            load_gtfs_data, GTFS_EXTRACT_DIR, timeout=PROCESSING_TIMEOUT
        )
        ...
        try:
            ...
            stops = process_with_timeout(
                process_stops, gtfs_data, max_stops, timeout=PROCESSING_TIMEOUT
            )
