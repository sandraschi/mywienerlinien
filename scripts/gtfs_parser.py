...
def print_route_info(schedule: pygtfs.Schedule) -> None:
    ...
    routes = (
        schedule.session.query(pygtfs.gtfs_entities.Route)
        .order_by(pygtfs.gtfs_entities.Route.route_short_name)
        .all()
    )
...
def print_stop_info(schedule: pygtfs.Schedule, route_short_name: str) -> None:
    ...
    route = (
        schedule.session.query(pygtfs.gtfs_entities.Route)
        .filter_by(route_short_name=route_short_name)
        .first()
    )
    ...
    trips = (
        schedule.session.query(pygtfs.gtfs_entities.Trip)
        .filter_by(route_id=route.route_id)
        .limit(1)
        .all()
    )  # Just get one trip for demonstration
    ...
    stop_times = (
        schedule.session.query(pygtfs.gtfs_entities.StopTime)
