...
class GTFSMarkdownGenerator:
    ...
    def _init_database(self) -> None:
        ...
        try:
            ...
            engine = create_engine(
                db_url, pool_pre_ping=True, connect_args={"timeout": 30, "check_same_thread": False}
            )
    ...
    def _generate_line_overviews(self, routes_by_type: dict[str, list[Any]]) -> None:
        ...
        try:
            for route_type, routes in routes_by_type.items():
                type_info = self.route_types.get(
                    route_type,
                    {
                ...
                for line_num, line_routes in sorted(lines.items()):
                    ...
                    if len(line_routes) > 1:
                        ...
                        for variant in line_routes:
                            content.append(
                                f"- {variant.trips[0].trip_headsign if variant.trips else 'Unknown'}"
                            )
                    ...
                    content.extend(
                        [
                            ...
                            f"- **Color**: `#{route.route_color}`"
                            if hasattr(route, "route_color") and route.route_color
                            else "",
                            "",
                        ]
                    )
            ...
            for route_type, routes in routes_by_type.items():
                ...
                for route in routes:
                    ...
                    try:
                        ...
                        trips = (
                            self.session.query(pygtfs.gtfs_entities.Trip)
                        ...
                        stop_times = (
                            self.session.query(pygtfs.gtfs_entities.StopTime)
                            .filter_by(trip_id=trips[0].trip_id)
                        ...
                        for i, stop_time in enumerate(stop_times, 1):
                            ...
                            # Add transfer info
                            transfers = (
                                self.session.query(pygtfs.gtfs_entities.Transfer)
                                .filter_by(from_stop_id=stop.stop_id)
                                .all()
                            )
                            ...
                            if transfers:
                                transfer_lines = []
                                for t in transfers:
                                    to_stop = self.session.query(pygtfs.gtfs_entities.Stop).get(
                                        t.to_stop_id
                                    )
                                    if to_stop:
                                        transfer_lines.append(
                                            f"  - Transfer to {to_stop.stop_name} (Walk: {t.min_transfer_time}s)"
                                        )
                        ...
                        # Write to file
                        safe_name = "".join(
                            c if c.isalnum() else "_" for c in route.route_short_name
                        )
                        ...
                        logger.error(
                            f"Error processing route {route.route_short_name}: {e}", exc_info=True
                        )
                        continue
    ...
    def _generate_stop_documents(self) -> None:
        ...
        try:
            ...
            stops = (
                self.session.query(pygtfs.gtfs_entities.Stop)
                .filter(
            ...
            for stop in stops:
                try:
                    ...
                    child_stops = (
                        self.session.query(pygtfs.gtfs_entities.Stop)
                        .filter_by(parent_station=stop.stop_id)
                        .all()
                    )
                    ...
                    for child in [stop] + child_stops:
                        stop_times = (
                            self.session.query(pygtfs.gtfs_entities.StopTime)
                            .filter_by(stop_id=child.stop_id)
                            .all()
                        )
                        ...
                        for st in stop_times:
                            try:
                                ...
                                feed_id = getattr(st, "feed_id", None) or getattr(
                                    self, "feed_id", None
                                )

                                if feed_id is not None:
                                    # Use composite key (feed_id, trip_id)
                                    trip = self.session.query(pygtfs.gtfs_entities.Trip).get(
                                        (feed_id, st.trip_id)
                                    )
                                else:
                                ...
                                logger.warning(
                                    f"Error processing trip {st.trip_id} for stop {child.stop_id}: {e}"
                                )
                                continue
                    ...
                    if routes:
                        ...
                        for route_num, route_type, route_name in sorted(routes):
                            if route_num:
                                type_info = self.route_types.get(
                                    str(route_type), {"name": f"Type {route_type}", "icon": "🚋"}
                                )
                                content.append(
                                    f"- {type_info.get('icon', '🚋')} **{route_num}**: {route_name} ({type_info['name']})"
                                )
