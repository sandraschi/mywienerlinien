...
def generate_station_markdown(station: dict, routes: list[dict]) -> str:
    ...
    if routes:
        ...
        for route_type in sorted(routes_by_type.keys()):
            ...
            for route in sorted(
                routes_by_type[route_type],
                key=lambda x: (x.get("route_short_name", ""), x.get("route_long_name", "")),
            ):
                name = route.get("route_short_name") or route.get(
                    "route_long_name", "Unnamed Route"
                )
                lines.append(f"- {name}")
...
def main():
    ...
    try:
        ...
        for i, station in enumerate(stations, 1):
            ...
            # Sanitize filename
            safe_name = "".join(c if c.isalnum() or c in " _-" else "_" for c in stop_name)
        ...
        logger.info(
            "Successfully generated %d station markdown files in %s", len(stations), output_dir
        )
